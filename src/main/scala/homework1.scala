...
object homework1 {
  ...
  def main(args: Array[String]) {
    ...
    val result5 = test5(host, msgPortUdp)
		...
		val topBar = for(i <- 0 to 51) yield {
			if(i == 0)
				"Test"
			else if(i == 51)
				"Average"
    ...
    for (i <- fields.indices) {
      ...
      for (j <- list.indices) {
        outFile.write(list(j).toString)
        //				if (j != list.length - 1)
        outFile.write(',')

      }
			outFile.write((list.sum/list.length).toString)
      outFile.newLine
    }
    outFile.close
  ...
  def test3(host: String, port: Int): Array[Array[Long]] = { //Value in bytes per second
    val output = Array.ofDim[Long](5, n)
    ...
    for (a <- output.indices) {
      for (b <- output(a).indices) {
        ...
        while (!validated) {
          ...
          if (bytesMatch(decoded, value)) {
            validated = true
            output(a)(b) = (byteNum.toLong * 1000000000) / ((received - sent) / 2).toLong
            println("Sent and received " + byteNum + " bytes in " + (received - sent) + " nanoseconds. Calculated " + output(a)(b) + " bytes per second")
          }
          else
